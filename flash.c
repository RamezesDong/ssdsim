...
{
	long long time=0;
	get_ppn(ssd,sub->location->channel,sub->location->chip,sub->location->die,sub->location->plane,sub); // 得到 ppn 得到page类型
	...
	sub->complete_time=sub->next_state_predict_time+ssd->parameter->time_characteristics.tPROG;		
...
int page_read_time(struct ssd_info *ssd, unsigned int page_loc)
{
	int tlc_time=0;
	int page_type_tlc = 0;
	page_type_tlc = page_type(ssd,page_loc);
...
struct ssd_info *compute_serve_time(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,struct sub_request **subs, unsigned int subs_count,unsigned int command)
{
	...
	else if(command==TWO_PLANE)
	{
		for(i=0;i<max_subs_num;i++)
		{
			if(subs[i]!=NULL)
			{
				...
				{
					subs[i]->current_time=time+ssd->parameter->time_characteristics.tDBSY;
				}
				...
				subs[i]->complete_time=subs[i]->next_state_predict_time;
				last_sub=subs[i];
				time = last_sub->complete_time;
				ssd->parameter->time_characteristics.tPROG = page_program_time(ssd, last_sub->location->page);
				subs[i]->complete_time = subs[i]->complete_time+ssd->parameter->time_characteristics.tPROG;
				delete_from_channel(ssd,channel,subs[i]);
			}
		}
		ssd->channel_head[channel].current_state=CHANNEL_TRANSFER;										
	...
	else if(command==NORMAL)
	{
		...
		time = subs[0]->next_state_predict_time;		
		ssd->parameter->time_characteristics.tPROG = page_program_time(ssd, subs[0]->location->page);
		...
		{	
			...
			{   
			    ...
			    ******************************************************************************************************/
				...
				ssd->parameter->time_characteristics.tR = page_read_time(ssd,sub->location->page);
