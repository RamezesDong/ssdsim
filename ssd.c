...
******************************************************************************/
	struct ssd_info *make_aged(struct ssd_info *ssd)
	{
		...
		{
			//threshold卤铆脢戮脪禄赂枚plane脰脨脫脨露脿脡脵脪鲁脨猫脪陋脤谩脟掳脰脙脦陋脢搂脨搂
				...
				for (j=0;j<ssd->parameter->chip_channel[i];j++)
					for (k=0;k<ssd->parameter->die_chip;k++)
						...
						{  
							...
							for (m=0;m<ssd->parameter->block_plane;m++)
							{  
								if (flag>threshould)	//flag 鏃犳晥涓暟锛岃嫢褰撳墠鏃犳晥涓暟澶т簬绛変簬闃堝�硷紝鍒欎笉鍐嶈繘琛寃arm up
								{
									break;
								}
								...
								{  
									rand_value = rand()%10;
									valid_tag = ((rand_value>2)? 1 : 0);	//0 for invalid   1 for valid	70% 鏈夋晥 30%鏃犳晥鏁版嵁
									
									
									if((valid_tag == 0) /*&& (invalid_num< (0.2*threshould+ 1)))||(valid_num >0.8*threshould)*/)
									{
										
										ssd->channel_head[i].chip_head[j].die_head[k].plane_head[l].blk_head[m].page_head[n].valid_state=0; 	   //卤铆脢戮脛鲁脪禄脪鲁脢搂脨搂拢卢脥卢脢卤卤锚录脟valid潞脥free脳麓脤卢露录脦陋0
										ssd->channel_head[i].chip_head[j].die_head[k].plane_head[l].blk_head[m].page_head[n].free_state=0;		   //卤铆脢戮脛鲁脪禄脪鲁脢搂脨搂拢卢脥卢脢卤卤锚录脟valid潞脥free脳麓脤卢露录脦陋0
										ssd->channel_head[i].chip_head[j].die_head[k].plane_head[l].blk_head[m].page_head[n].lpn=0;  //掳脩valid_state free_state lpn露录脰脙脦陋0卤铆脢戮脪鲁脢搂脨搂拢卢录矛虏芒碌脛脢卤潞貌脠媒脧卯露录录矛虏芒拢卢碌楼露脌lpn=0驴脡脪脭脢脟脫脨脨搂脪鲁
									...
									else
									{
										...
										lpn = count;	//椤哄簭浜х敓杩炵画lpn锛屼粎瀵规湁鏁堟暟鎹繘琛宭pn閫掑
										count++;
