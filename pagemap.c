...
int uninterrupt_gc(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane)       
{
	...
	LSB_program = page_program_time(ssd,0);
	CSB_program = page_program_time(ssd,2);
	MSB_program = page_program_time(ssd,5);
	LSB_read = page_read_time(ssd,0);
	CSB_read = page_read_time(ssd,2);
	MSB_read = page_read_time(ssd,5);
	...
	***************************************************************/
	...
	//the difference of tlc pages has not been considered during gc process  gaocm
	if ((ssd->parameter->advanced_commands&AD_COPYBACK)==AD_COPYBACK)
	{
...
unsigned int gc(struct ssd_info *ssd,unsigned int channel, unsigned int flag)
{
	...
	if (flag==1)                                                                       /*整个ssd都是IDEL的情况*/
	{
		for (i=0;i<ssd->parameter->channel_number;i++)
		{
			...
			{
				channel=i;
				if (ssd->channel_head[channel].gc_command!=NULL) // gc_command 记录挂载在该channel上的请求
				{
